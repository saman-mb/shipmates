...
test -f "$RUNNER_TEMP/install-all/.agents/skills/ship-issue/SKILL.md"
...
if grep -qE '\.(agents|codex|cursor|github|pi)/agents' "$RUNNER_TEMP/install-all/.agents/skills/ship-issue/SKILL.md"; then
  echo "FAIL: shared skills tree names a harness-specific crew tree" >&2
  exit 1
fi
grep -q 'shipped crew role' "$RUNNER_TEMP/install-all/.agents/skills/ship-issue/SKILL.md"

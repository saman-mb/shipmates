...
test -f "$RUNNER_TEMP/install-all/.agents/skills/shipmates-ship-issue/SKILL.md"
# grok-build is crew-bearing on its own tree, not the shared one
test -f "$RUNNER_TEMP/install-all/.grok/skills/shipmates-ship-issue/SKILL.md"
...
if grep -qE '\.(agents|codex|cursor|github|pi)/agents' "$RUNNER_TEMP/install-all/.agents/skills/shipmates-ship-issue/SKILL.md"; then
  echo "FAIL: shared skills tree names a harness-specific crew tree" >&2
  exit 1
fi
grep -q 'shipped crew role' "$RUNNER_TEMP/install-all/.agents/skills/shipmates-ship-issue/SKILL.md"

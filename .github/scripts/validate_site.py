...
JSON-LD, stage counts must agree across source, markup and JSON-LD, and the
page is a *guide* (COPY blurbs and stage summaries) — not a dump of the skill.
...
C3_CLASSES = (
    ...
    "order-when", "order-when__list",
    "order-process", "order-process__list", "order-process__item",
    "order-process__num", "order-process__body", "order-process__label",
    "order-process__line",
    "order-process__lead",
    "order-process__crew", "order-process__crew--solo", "order-process__chip",
...
def check_command_page(page: Page) -> None:
    ...
    check_process_parity(page, data)
    check_process_crew(page)

    check_command_is_guide(page)
...
def check_ldjson_howto(page: Page) -> dict | None:
    ...
    for i, step in enumerate(steps, start=1):
        ...
        if not isinstance(url, str) or not url.startswith(page.url + "#"):
            fail(f"{where} url is {url!r}, expected {page.url!r} + a fragment")
            bad += 1
...
def check_process_crew(page: Page) -> None:
    """Every How it works step names who sits — chips, or a no-spawn note."""
    n_items = page.collector.classes.count("order-process__item")
    n_crew = page.collector.classes.count("order-process__crew")
    if n_items == n_crew and n_items > 0:
        ok(f"{page.rel}: process crew: {n_crew} steps name who sits")
    else:
        fail(
            f"{page.rel}: process crew mismatch — {n_items} .order-process__item, "
            f"{n_crew} .order-process__crew (every step must name the crew or say the run does it)"
        )


def check_process_parity(page: Page, data: dict | None) -> None:
    """Guide process steps in markup == JSON-LD HowTo.step (not skill stage count)."""
    n_markup = page.collector.classes.count("order-process__item")
    n_steps = len(data.get("step", [])) if isinstance(data, dict) and isinstance(data.get("step"), list) else None
    if n_steps is None:
        fail(f"{page.rel}: cannot check process parity — no usable JSON-LD step list")
        return
    if n_markup == n_steps and 4 <= n_markup <= 6:
        ok(f"{page.rel}: process parity: {n_markup} steps in markup and JSON-LD")
    else:
        fail(
            f"{page.rel}: process step mismatch — page has {n_markup} .order-process__item, "
            f"JSON-LD lists {n_steps} step(s) (expect 4–6) — {REGEN_HINT}"
        )
...
def check_command_is_guide(page: Page) -> None:
    """Command pages summarise; they must not dump the install skill onto the site."""
    ids = set(page.collector.ids)
    required = ("when-to-use", "process")
    absent = [s for s in required if s not in ids]
    if absent:
        fail(f"{page.rel}: missing guide section(s): {', '.join('#' + s for s in absent)}")
    else:
        ok(f"{page.rel}: guide sections present (how it works, when to use)")
    banned = ("advanced", "guardrails", "stages", "config")
    leaked = [s for s in banned if s in ids]
    if leaked:
        fail(
            f"{page.rel}: skill dump leaked onto the guide page — "
            f"{', '.join('#' + s for s in leaked)} (keep the full skill on GitHub)"
        )

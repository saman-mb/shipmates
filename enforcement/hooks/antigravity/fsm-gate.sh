#!/usr/bin/env bash
#
# Shipmates FSM tool-gate — Antigravity (`agy`) `PreToolUse` hook.
#
# Turns a `shipmates state gate` verdict into an Antigravity permission decision.
# Antigravity `PreToolUse` fires for every tool; its `.agents/hooks.json` matcher
# is a tool-name regex, but the shim ALSO checks the tool-name defensively and
# gates only the shell tool (`run_command`). It only ever GATES a run it can
...
#               deny  → emit `decision: "deny"` on stdout (exit 0, so the
#                       decision is honoured rather than read as a hook fault).
#               allow → exit 0, no output.
#               error → allow + log to stderr (fail-safe; an engine fault must
#                       not wedge the session).
#
# Deny form: stdout JSON with `decision: "deny"` (and a `reason`), exit 0.
#
...
if [ "${SHIPMATES_NATIVE_HOOK:-}" = "1" ]; then
    exec shipmates hook gate --harness antigravity
fi
...
case "$code" in
    0)
        ...
        ;;
    1)
        # Deny — emit Antigravity's decision on stdout. Exit 0 so agy honours the
        # decision rather than treating a non-zero exit as a fault.

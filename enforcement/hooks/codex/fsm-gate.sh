#!/usr/bin/env bash
#
# Shipmates FSM tool-gate — Codex `PreToolUse` hook (experimental).
#
# Turns a `shipmates state gate` verdict into a Codex permission decision. Codex
# `PreToolUse` fires for every tool, so the shim gates ONLY the shell tool and
# stays silent on everything else. It only ever GATES a run it can unambiguously
# identify; in every other case it stays silent and allows, so it can never block
...
# name against `shell`/`local_shell`/`bash`/`exec`, and the command is read from
# `.tool_input.command`. If Codex's real event shape differs, an unrecognised
# tool-name or missing command falls through to a fail-safe ALLOW rather than a
# wrong block. Enabling hooks at all requires `config.toml [features].codex_hooks
# = true`; wiring that flag (and the `.codex/hooks/` registration) is #217.
#
# Requires `jq` and `git`.
...
case "$tool_name" in
    shell | local_shell | bash | exec | Bash) ;;
    *) exit 0 ;;
esac

...
Resolve every role name against `.claude/agents/*.md`; on miss, fall back to `general-purpose` with
...
- If a role doesn't resolve to an `.claude/agents/*.md`, fall back to `general-purpose` with the

...
assert_file_count "$PROJ" 51 "claude-code all-tools: 51 files"

PROJ="$TMPDIR/proj-counts-scrub"
mkdir -p "$PROJ"
cmd_run "$BIN" install --harness claude-code --dir "$PROJ" --with-tools scrub >/dev/null 2>&1
assert_file_count "$PROJ" 31 "claude-code scrub tool: 31 files"
...
assert_file_count "$PROJ" 29 "Files unchanged after re-install"
...
[ "$CODEX_FILES" -eq 29 ] && ok "Codex install: 29 files" || fail "Codex install: $CODEX_FILES files (expected 29)"
...
[ "$SHARED_FILES" -eq 43 ] && ok "After github-copilot: 43 files" || fail "After github-copilot: $SHARED_FILES files (expected 43)"
...
assert_contains "28 files written" "$CMD_OUT" "--force reports 28 files (overwrote legacy)"

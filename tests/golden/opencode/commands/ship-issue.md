...
Every specialist below is a **named subagent** shipped alongside this command (`.opencode/agents/*.md`,
installed globally or per-project) and invoked by its `subagent_type` — NOT a `general`
...
not resolve to a `.opencode/agents/*.md`, fall back to `general` with the role's brief inlined,
...
## Stage 0 — Intake & plan  (agent: `architect`)
...
3. Spawn ONE **Planner** (`subagent_type: architect`). Give it all issue bodies + repo README +

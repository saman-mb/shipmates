...
- `TARGET` = auto-detected. `AGENTS.md` if one exists, else `CLAUDE.md` if one exists, else
  `AGENTS.md`. **Never write both** — see below.
...
an unpushed `AGENTS.md`/`CLAUDE.md` that Stage 0's survey already saw is actually present in the
...
- If a role doesn't resolve to a `.opencode/agents/*.md`, fall back to `general` with the brief
  inlined, and note it.

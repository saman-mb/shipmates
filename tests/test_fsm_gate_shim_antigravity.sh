#!/usr/bin/env bash
#
# End-to-end test for the Antigravity `PreToolUse` FSM tool-gate shim
# (enforcement/hooks/antigravity/fsm-gate.sh). Simulated event payloads on stdin
# against a throwaway git repo + run file, with a STUBBED `shipmates` on PATH
# whose exit code we control (0 allow / 1 deny / 2 error).
#
# Deny form (Antigravity): stdout JSON with `decision: "deny"`, exit 0.
#
#   bash tests/test_fsm_gate_shim_antigravity.sh
#
...
SHIM="$REPO/enforcement/hooks/antigravity/fsm-gate.sh"
...
# agy's shell tool is `run_command`; PreToolUse fires for every tool.
shell_payload() { jq -cn --arg cmd "$1" --arg cwd "$GIT" '{tool_name:"run_command", tool_input:{command:$cmd}, cwd:$cwd}'; }
other_payload() { jq -cn --arg cwd "$GIT" '{tool_name:"view_file", tool_input:{path:"/etc/hosts"}, cwd:$cwd}'; }
...
# (a) engine deny (exit 1) → decision:deny on stdout, exit 0.
export STUB_EXIT=1
run_shell 'gh pr merge --squash'
if printf '%s' "$OUT" | jq -e '.decision == "deny"' >/dev/null 2>&1 && [ "$RC" -eq 0 ]; then
    ok "(a) deny → decision:deny, exit 0"
else
    bad "(a) expected decision=deny, got rc=$RC out=$OUT"
fi

#!/usr/bin/env bash
#
# End-to-end test for the GitHub Copilot `preToolUse` FSM tool-gate shim
# (enforcement/hooks/github-copilot/fsm-gate.sh). Simulated event payloads on
# stdin against a throwaway git repo + run file, with a STUBBED `shipmates` on
# PATH whose exit code we control (0 allow / 1 deny / 2 error).
#
# Deny form (Copilot): stdout JSON with `permissionDecision: "deny"`, exit 0.
# (The Copilot deny channel is not yet verified live — see the shim's README.)
#
#   bash tests/test_fsm_gate_shim_github-copilot.sh
#
...
SHIM="$REPO/enforcement/hooks/github-copilot/fsm-gate.sh"
...
# Copilot's shell tool maps to `execute`; preToolUse fires for every tool.
shell_payload() { jq -cn --arg cmd "$1" --arg cwd "$GIT" '{tool_name:"execute", tool_input:{command:$cmd}, cwd:$cwd}'; }
other_payload() { jq -cn --arg cwd "$GIT" '{tool_name:"read", tool_input:{path:"/etc/hosts"}, cwd:$cwd}'; }
...
# (d) non-shell tool → allow even on a deny verdict.
export STUB_EXIT=1

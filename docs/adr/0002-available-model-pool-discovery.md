...
| opencode | query | a listing subcommand taking an optional provider, with a cache-refresh and a verbose flag | static agent file, plus global config and a session flag | run-level; provider-defined, no fixed enum | provider-level only (`enabled_providers` / `disabled_providers`, denylist wins, silent exclusion) |
...
### Corrections — where the issue's own table is contradicted by first-party docs
...
| github-copilot effort | no per-agent effort field | **contradicted** — the custom-agents reference documents an agent-frontmatter reasoning-effort key (plus a flag and a settings key, with mismatched vocabularies) | `docs.github.com/en/copilot/reference/custom-agents-configuration` |
...
| antigravity / github-copilot / windsurf effort | absent | **conflated** — static per-agent effort is absent on all three (the CI-enforced `effort` flag stays `false`), but a routing-usable override is present at run level on the first two; only the third has none | `antigravity.google/docs/cli/headless`; the two Copilot references above; `docs.devin.ai/cli/models` |

...
| opencode / pi effort surface | a separate static key on opencode; a run-level override on pi | **reclassified** — a variant folded into the model argument on opencode; a separate per-session key with a 7-step scale on pi (the doc sets disagree with the story's mapping in both directions) | `opencode.ai/docs/models`; `opencode.ai/docs/cli`; pi's own `docs/usage.md`, `docs/settings.md`, `docs/models.md` |

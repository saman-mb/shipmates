...
- **Claude Code today; seven more targets build but are not runtime-verified.** Claude Code is the only harness Shipmates has been *run* on. opencode, Gemini/Antigravity, Codex CLI, Cursor, GitHub Copilot, Windsurf and Zed each have an adapter — `shipmates install --harness <name>` produces their trees, every payload's format was checked against the harness's own parsing source and first-party docs, and each target's digest is gate-checked in CI. opencode and Gemini/Antigravity receive the full crew + all 12 commands (they have native subagent directories); the other five ship the 12 skills only. None has been **verified against a running harness**: whether agents resolve, whether argument passing behaves, and whether `/ship-issue` completes end to end are open. opencode's are tracked in [#31](https://github.com/saman-mb/shipmates/issues/31) and [#32](https://github.com/saman-mb/shipmates/issues/32). Do not write "tested on <harness>" anywhere until those close. See the roadmap on the [website](https://saman-mb.github.io/shipmates/#next) and in [`README.md`](README.md#-on-the-horizon).

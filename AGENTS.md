...
- **Adding a frontmatter key to a canonical skill?** You must add a matrix entry per harness in `tools/harness_matrix.json`, or the exporter/generator gate (and CI) fails. `harnesses/<target>/` trees are **generated** and committed — never hand-edit them; run the target exporter and its `--check` command. CI enforces drift checks.

...
- **Adding a frontmatter key to a canonical skill?** You must add a matrix entry per harness in `tools/harness_matrix.json`, or `tools/build_harness_payloads.py` (and CI) fails. `harnesses/<target>/` trees are **generated** by `tools/build_harness_payloads.py` and committed — never hand-edit them; run `python3 tools/build_harness_payloads.py` and commit the result. CI enforces this with `--check`.

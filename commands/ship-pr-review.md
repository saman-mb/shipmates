...
---
# /ship-pr-review — classify → convene the board → consolidate
...
`/ship-harden` isn't an available remedy. `/ship-issue` keeps the same flag (it still gates the `/ship-harden`
recommendation, and forces a manual merge on **standalone** runs onto the default branch) but not the
seat, since a crew-authored change can just run `/ship-harden` itself. Epic-delegated units
...
flags for scaled optional seats. **`/ship-pr-review`-specific additions** on top of the shared board:
...
- **Read-only by default.** No worktree, no commits, no pushes, no fix loop. If the findings need
  fixing, hand them to `/ship-fix-bug` or `/ship-issue` — don't fork a remediation loop into this command.
- **This command crosses a trust boundary the others don't.** `/ship-issue`, `/ship-fix-bug` and `/ship-migrate`
